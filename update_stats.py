#!/usr/bin/env python3
"""
Build and refresh the NVIDIA GitHub repository catalog.

The script fetches public GitHub metadata for NVIDIA repositories that were
updated in the last three months and have at least 200 stars, groups them into
...
USER_AGENT = "nvidia-repos-catalog"
AUTO_KEYWORD_TERMS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ("DGX Spark", ("dgx spark", "dgx-spark", "dgx_spark")),
    ("Thor", ("thor",)),
    ("Jetson", ("jetson",)),
    ("Orin", ("orin",)),
    ("AGX", ("agx",)),
    ...
    ("DALI", ("dali",)),
    ("DCGM", ("dcgm",)),
    ("JetPack", ("jetpack", "jet pack", "jet-pack")),
    ("ROS", ("ros",)),
...
CLUSTERS: tuple[Cluster, ...] = (
    Cluster(
        "ai-llm",
        "AI, LLM & Model Tooling",
        "Training, inference, LLM serving, model optimization, and applied AI libraries.",
        "green",
        (
            "ai",
            "agent",
    ...
    Cluster(
        "gpu-systems",
        "GPU Computing, CUDA & Systems",
        "CUDA samples, drivers, kernels, GPU libraries, compilers, and performance code.",
        "blue",
        (
            "cuda",
            "cublas",
    ...
    Cluster(
        ...
        "Isaac, Jetson, robotics, simulation, digital twins, autonomy, and edge deployments.",
        "orange",
        (
            "autonomous",
            "digital twin",
    ...
    Cluster(
        "data-analytics",
        "Data Science, RAPIDS & Analytics",
        "RAPIDS, accelerated dataframe tooling, ML analytics, recommender systems, and data pipelines.",
        "cyan",
        (
            "analytics",
            "cudf",
    ...
    Cluster(
        ...
        "Kubernetes, containers, operators, MLOps, orchestration, networking, and service infrastructure.",
        "purple",
        (
            "cloud",
            "container",
    ...
    Cluster(
        "graphics-vision-media",
        "Graphics, Vision & Media",
        "RTX, rendering, computer vision, video, imaging, Vulkan/OpenGL, and media pipelines.",
        "pink",
        (
            "3d",
            "camera",
...
EXTRA_SECTIONS: tuple[SourceSection, ...] = (
    SourceSection(
        "nvidia-ai-iot",
        "NVIDIA-AI-IOT Repositories",
        "Active repositories from the NVIDIA-AI-IOT GitHub organization with commits in the last three months.",
        "org:NVIDIA-AI-IOT archived:false",
        "orange",
    ),
    SourceSection(
        "jetsonhacks",
        "JetsonHacks Repositories",
        "Active JetsonHacks repositories with commits in the last three months.",
        "user:jetsonhacks archived:false",
        "cyan",
    ),
    SourceSection(
        "dusty-nv",
        "dusty-nv Repositories",
        "Active dusty-nv repositories with commits in the last three months.",
        "user:dusty-nv archived:false",
        "purple",
    ),
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Update NVIDIA GitHub repository catalog.")
...
def traction_score(repo: dict[str, Any]) -> float:
    ...
    if repo.get("has_history_baseline"):
        ...
        return (star_delta * 10) + (fork_delta * 4) + min(commits, 250)
    ...
    star_weight = math.log10(max(repo["stars"], 10)) * 12
    return min(commits, 250) + (freshness * star_weight)
...
<section class="repo-section traction-section" id="section-traction" data-section>
  <div class="section-header">
    <div>
      <h2><span class="section-mark section-mark-green"></span>Most Traction In The Last {days} Days</h2>
      <p>Ranked by recent commits and activity score. The bar compares each repository against the top entry in this table.</p>
    </div>
...
def jump_links(extra_sections: dict[str, list[dict[str, Any]]]) -> str:
    cluster_items = [
        ("section-traction", "Most Traction", None),
    ...
    return "\n".join(
        [
            render_group("Clusters", cluster_items, "jump-group-clusters"),
            render_group("Other Repos", extra_items, "jump-group-other"),
        ]
    )
...
<title>NVIDIA GitHub Repository Atlas</title>
...
<style>
  ...
  * {{ box-sizing: border-box; }}
  ...
  body {{
    ...
    min-height: 100vh;
    background:
      radial-gradient(circle at 85% -10%, rgba(118, 185, 0, 0.18), transparent 28rem),
      linear-gradient(180deg, #0b1010 0%, var(--bg) 42rem);
    color: var(--text);
  ...
  .jump-group-other {{
    border-color: rgba(49, 213, 216, 0.22);
    background: rgba(49, 213, 216, 0.045);
  }}

  .jump-group-other a {{
    border-color: rgba(49, 213, 216, 0.36);
  ...
  .jump-group-other a:hover {{
    ...
    color: var(--cyan);
    background: rgba(49, 213, 216, 0.12);
  }}
  ...
  .search-panel {{
    ...
    border-radius: 12px;
    background:
      radial-gradient(circle at top right, rgba(118, 185, 0, 0.14), transparent 24rem),
  ...
  .section-mark-green {{ background: var(--nvidia); }}
  ...
  tbody tr:hover {{ background: rgba(118, 185, 0, 0.035); }}
  ...
  .tag-py {{ background: rgba(118, 185, 0, 0.13); color: var(--nvidia); }}
  ...
  .tag-go {{ background: rgba(49, 213, 216, 0.13); color: var(--cyan); }}
...
<body>
  <header class="header page-header">
    <h1 class="page-title"><span class="logo-box">NV</span>NVIDIA GitHub Repository Atlas</h1>
    <p>Searchable snapshot of public NVIDIA repositories from <a href="https://github.com/orgs/NVIDIA/repositories" target="_blank" rel="noreferrer">github.com/NVIDIA</a>. Included repositories have at least {args.min_stars} stars and a commit within the last three months, since {cutoff}.</p>
...
{extra_section_html}
  ...
  <footer class="footer">
    <p style="margin-bottom: 10px;">Useful links:
      <a class="gh-link" href="https://www.jetson-ai-lab.com/" target="_blank" rel="noreferrer" style="display: inline; margin-left: 6px;">jetson-ai-lab.com</a>
      <span style="margin: 0 4px;">·</span>
      <a class="gh-link" href="https://jetsonhacks.com/" target="_blank" rel="noreferrer" style="display: inline;">jetsonhacks.com</a>
    </p>
    <p style="text-align: center;">&copy; <span id="copyright-year"></span> Robert Zaufall · Last updated: <span id="last-updated-date">{updated}</span></p>
  </footer>
...
def main() -> int:
    ...
    print(f"Wrote {args.file} with {len(repos)} NVIDIA repositories and {len(all_repos) - len(repos)} adjacent repositories")
